...
switch ($act) {
    ...
    case 'view-entry':
        ...
        break;
    case 'do-new-entry':
        if (!IS_ADMIN) die("Вы должны быть администратором, чтобы добавить запись.");
        require_once(__DIR__ . "/model/Entry.php");
        $Entry = new Entry();
        if ($Entry->add($_POST['author'], $_POST['header'], $_POST['content'])) {
            header('Location: .');
        } else {
            die("Невозможно добавить запись.");
        }
        break;
    case 'delete-entry':
        if (!IS_ADMIN) die("Вы должны быть администратором, чтобы удалить запись.");
        require_once(__DIR__ . "/model/Entry.php");
        $Entry = new Entry();
        $Entry->remove($_GET['id']) or die("Невозможно удалить запись.");
        header('Location: .');
        break;
    case 'delete-comment':
        if (!IS_ADMIN) die("Вы должны быть администратором, чтобы удалить комментарий.");
        require_once(__DIR__ . "/model/Comment.php");
        $Comment = new Comment();
        $Comment->remove($_GET['id']) or die("Невозможно удалить комментарий.");
        header('Location: ?act=view-entry&id=' . intval($_GET['entry_id']));
        break;
    case 'edit-entry':
        if (!IS_ADMIN) die("Вы должны быть администратором, чтобы изменить комментарии.");
        $id = intval($_GET['id']);
        require_once(__DIR__ . "/model/Entry.php");
        $Entry = new Entry();
